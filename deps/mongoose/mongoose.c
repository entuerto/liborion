...
void mg_if_connect_tcp(struct mg_connection *nc,
  ...
  MLOG(LL_INFO, ("%p sock %d err %d", nc, nc->sock, nc->err));
}
  ...
  switch (ev) {
    case MG_EV_HTTP_PART_BEGIN: {
      ...
      if (lfn.p == NULL || lfn.len == 0) {
        MLOG(LL_ERROR, ("%p Not allowed to upload %s", nc, mp->file_name));
        mg_printf(nc,
      ...
      if (fus->fp == NULL) {
        mg_printf(nc,
                  ...
                  "Content-Type: text/plain\r\n"
                  "Connection: close\r\n\r\n");
        MLOG(LL_ERROR, ("Failed to open %s: %d\n", fus->lfn, errno));
    ...
    case MG_EV_HTTP_PART_DATA: {
      ...
      if (fwrite(mp->data.p, 1, mp->data.len, fus->fp) != mp->data.len) {
        MLOG(LL_ERROR, ("Failed to write to %s: %d, wrote %d", fus->lfn, errno,
                       (int) fus->num_recd));
        if (errno == ENOSPC
      ...
      MLOG(LL_DEBUG, ("%p rec'd %d bytes, %d total", nc, (int) mp->data.len,
                     (int) fus->num_recd));
      break;
    }
    case MG_EV_HTTP_PART_END: {
      ...
      if (mp->status >= 0 && fus->fp != NULL) {
        MLOG(LL_DEBUG, ("%p Uploaded %s (%s), %d bytes", nc, mp->file_name,
                       fus->lfn, (int) fus->num_recd));
        mg_printf(nc,
                  "HTTP/1.1 200 OK\r\n"
                  "Content-Type: text/plain\r\n"
        ...
        MLOG(LL_ERROR, ("Failed to store %s (%s)", mp->file_name, fus->lfn));
        /*
