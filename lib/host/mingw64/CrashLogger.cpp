                     ...
                     int* error)
{
   if (section_index == 0)
   {
      // simulate the ELF empty section at index 0, as recommended in various comments in
      // dwarf_init_finish.c.
   ...
   if (return_section->name[0] == '/')
   {
      ...
      return_section->name = reinterpret_cast<const char*>(
         image->MappedAddress + header.PointerToSymbolTable + header.NumberOfSymbols * 18 +
         atoi(return_section->name + 1));
   }
   return_section->link = 0;
   return DW_DLV_OK;
}
       ...
       (tag == DW_TAG_subprogram || tag == DW_TAG_inlined_subroutine))
   {
          ...
          dwarf_attr(die, DW_AT_high_pc, &high_attr, &error) == DW_DLV_OK)
      {
         ...
         if (dwarf_whatform(high_attr, &form, &error) == DW_DLV_OK)
         {
            ...
            { // absolute ref, we can use dwarf_highpc.
      ...
      if (dwarf_whatform(attr, &form, &error) == DW_DLV_OK && form == DW_FORM_ref_sig8)
      {
         ...
         if (dwarf_formsig8(attr, &sig, &error) == DW_DLV_OK)
         {
            ...
            while (dwarf_next_cu_header_c(
                      dbg, 0, 0, 0, 0, 0, 0, 0, &cu_sig, &type_offset, &next, &error) == DW_DLV_OK)
            {
...
void getDebugInfo(string path, DWORD addr, string& file, int& line, int& column, string& function)
{
   ...
   Dwarf_Obj_Access_Methods methods  = {get_section_info,
                                       get_byte_order,
                                       get_length_size,
                                       get_pointer_size,
                                       get_section_count,
               ...
               if (dwarf_offdie_b(dbg, die_offset, 1, &cu_die, &error) == DW_DLV_OK)
               {
                  ...
                  if (die)
                  {
                     ...
                     if (!function.empty())
                     {
                        ...
                        function =
                           getType(dbg, spec_die ? spec_die : die, error) + ' ' + function + '(';
...
inline void writePlatformInfo()
{
   ...
   {
      fprintf(f,
              "Windows version: major = %d, minor = %d, build = %d, SP = %d, type = %d\n",
              ver.dwMajorVersion,
              ver.dwMinorVersion,
              ver.dwBuildNumber,
...
#define WALK_ARCH IMAGE_FILE_MACHINE_I386
   ...
   { // 128 steps max to avoid too long traces
      ...
      if (module.LoadedImageName[0] ||
          ...
          ::GetModuleFileNameA(reinterpret_cast<HMODULE>(module.BaseOfImage),
