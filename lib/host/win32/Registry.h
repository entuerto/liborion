...
public:
   ...
   std::error_code enum_value(std::vector<KeyValue>& values)
   {
      ...
      { 
         uint32_t ret = RegEnumKeyExW(_subkey, 
                                      i, 
                                      data, 
                                      reinterpret_cast<LPDWORD>(&ki.max_subkey_len),
                                      nullptr, 
                                      nullptr, 
                                      nullptr, 
                                      &last_write_time); 
      ...
      { 
         ...
         uint32_t ret = RegEnumValueW(_subkey, 
                                      i, 
                                      data, 
                                      reinterpret_cast<LPDWORD>(&data_len), 
                                      nullptr /* lpReserved */, 
                                      nullptr /* lpType */,
                                      nullptr /* lpData */,
                                      nullptr /* lpcbData */);
   ...
   std::error_code query_info_key(KeyInfo& ki)
   {
      ...
      uint32_t ret = RegQueryInfoKey(
         ...
         nullptr /* lpcClass */,
         nullptr /* lpReserved */,
         reinterpret_cast<LPDWORD>(&ki.subkey_count),
         reinterpret_cast<LPDWORD>(&ki.max_subkey_len),
         nullptr /* lpcMaxClassLen */,
         reinterpret_cast<LPDWORD>(&ki.value_count),
         reinterpret_cast<LPDWORD>(&ki.max_value_name_len),
         reinterpret_cast<LPDWORD>(&ki.max_value_len),
