...
do 
{
   ...
   ret = GetAdaptersAddresses(AF_UNSPEC, GAA_FLAG_INCLUDE_PREFIX, nullptr, addresses, &outBufLen);
...
if (ret != NO_ERROR)
{
    std::cout << "Call to GetAdaptersAddresses failed with error: "
              << ret
              << "\n";
   return EXIT_FAILURE;
}
